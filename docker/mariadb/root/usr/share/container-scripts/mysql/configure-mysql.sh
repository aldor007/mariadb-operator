...
mysqld --skip-networking --socket=/var/run/mysqld/mysql-init.sock --wsrep_on=OFF &
pid="$!"

mysql=( mysql --protocol=socket -uroot -hlocalhost --socket=/var/run/mysqld/mysql-init.sock)
...
GRANT SELECT, PROCESS, RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO 'xtrabackup_sst'@'localhost' ;
CREATE USER '${BACKUP_USER}'@'%' IDENTIFIED BY '${BACKUP_PASSWORD}' ;
GRANT SELECT, PROCESS, RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO '${BACKUP_USER}'@'%' ;
